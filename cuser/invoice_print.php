 ...
 * 🔒 فقط فاکتورهای خود گارسون قابل مشاهده هستند
 */
session_start();
...
$current_cafe_id = (int)($wrow['cafe_id'] ?? 0);
...
if ($invoice_id > 0) {
    ...
    if (mysqli_num_rows($db->res) > 0) {
        ...
        if (!empty($invoice['waiter_id']) && $invoice['waiter_id'] > 0) {
            $wid_row = (int)$invoice['waiter_id'];
            $db->query("SELECT fullname FROM `waiters` WHERE id = $wid_row");
            if (mysqli_num_rows($db->res) > 0) {
...
<div class="container-fluid px-3 py-3">
    ...
    <?php if (!$invoice) { ?>
        ...
        <div class="empty-state">
            ...
            <p>این فاکتور یا وجود ندارد یا متعلق به شما نیست.</p>
        </div>
