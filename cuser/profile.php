...
<!DOCTYPE html>
<html>
<title>ویرایش پروفایل گارسون</title>
<meta charset="UTF-8">
...
<?php
/* واکشی اطلاعات گارسون جاری از سشن */
$thisuser = $_SESSION['tel'] ?? '';
$dbc = new database();
$esc = function ($v) use ($dbc) {
    $link = null;
    foreach (['connection', 'link', 'conn', 'con', 'db', 'dbh', 'mysqli'] as $prop) {
        if (isset($dbc->$prop) && $dbc->$prop instanceof mysqli) {
            $link = $dbc->$prop;
            break;
        }
    }
    return $link ? mysqli_real_escape_string($link, $v) : $v;
};
$tel_s = $esc($thisuser);
$dbc->connect()->query("select * from `waiters` where `tel`='$tel_s' limit 1");
                ...
                <div class="card-body p-4">
                    ...
                    <form id="profile-form" onsubmit="return false;">
                        ...
                        <input type="text" name="fullname" id="fullname"
                               ...
                               value="<?= htmlspecialchars($fildc['fullname'] ?? '', ENT_QUOTES, 'UTF-8') ?>">

                        <label class="form-label">شماره تماس (نام کاربری)</label>
                        <input type="text" name="tel" id="tel"
                               class="form-control mb-3"
                               value="<?= htmlspecialchars($fildc['tel'] ?? '', ENT_QUOTES, 'UTF-8') ?>"
                               disabled>
