...
class makeform
{
    ...
    public function dateinput($name, $title, $req = 0, $show_in_tbl = 0, $filter = 0, $class = "")
    {
        ...
        $this->label($title, "form-label")
            ...
            ->inpclasses("form-control ta" . $name . " " . $class)
            ->end();
    ...
    public function submitbtn($val, $classes = "btn btn-success rounded m-2")
    {
        $this->input()
            ->inptype("submit")
            ->inpval($val)
    ...
    public function show()
    {
        ...
        if (isset($_GET['action']) == false && $this->alow_add == true) {
            ...
            if ($this->alow_visit == true) {
                ...
                echo($fl->getfilename()); ?>?action=show"><input type="button" class="btn btn-primary rounded m-2"
                                                                 value="مشاهده"></a>
                <?php
            }
            echo($this->all);
        ...
        ($_GET['action'] == true) {
            if ($_GET['action'] == "addform" && $this->alow_add == true) {
                ...
                if ($this->alow_add != false) {
                    ?>
                    <a href="<?php $fl = new filemg();
                    echo($fl->getfilename()); ?>"><input type="button" class="btn btn-success rounded m-2"
                                                         value="+افزودن"></a>
                    <?php
                }
                $have_filter = 0;
                ...
                if ($have_filter > 0) {
                    ?>
                    <input type="button" class="btn btn-warning rounded" value="فیلترینگ!"
                           onclick="hidefilter();">
                    <?php
                }
                $this->loadfilter();

                $restbl = "<div style='width: 100%; overflow: scroll;'><table border='1' class='table table-striped' style='width: 100%;'><tr>";
                for ($i = 0; $i < sizeof($this->formtbl); $i++) {
                    if ($this->formtbl[$i] == 1) {
                        $restbl .= "<th class='text-end'>" . $this->formtitle[$i] . "</th>";
                    }
                }
                $restbl .= "<th class='text-end'>عملیات</th>";
                $restbl .= "</tr>";
                    ...
                    for ($i = 0; $i < sizeof($this->formtbl); $i++) {
                        if ($this->formtbl[$i] == 1) {
                            if ($this->formtype[$i] == 2) {
                                $restbl .= "<td class='text-end'>" . $this->selectvals[$this->formname[$i]][$fild[$this->formname[$i]]] . "</td>";
                            } elseif ($this->formtype[$i] == 5) {
                                ...
                                $restbl .= "<td class='text-end'>" . $jfdate . "</td>";
                            } else {
                                $restbl .= "<td class='text-end'>" . $fild[$this->formname[$i]] . "</td>";
                            }
                        }
                    }
                    $restbl .= "<td class='text-end'>";
                    if ($this->alow_edit == true) {
                        $restbl .= " <a href='" . $fl->getfilename() . "?action=editform&" . $this->setkey . "=" . $fild[$this->setkey] . "'><input type='button' value='ویرایش' class='btn btn-primary rounded'></a> ";
                    }
                    if ($this->alow_del == true) {
                        $restbl .= " <a href='" . $fl->getfilename() . "?action=deletequery&csrf_token=" . $rand . "&" . $this->setkey . "=" . $fild[$this->setkey] . "'><input type='button' value='حذف' class='btn btn-danger rounded'></a> ";
                    }
    ...
    function loadfilter()
    {
        ...
        for ($i = 0; $i < sizeof($this->formfilter); $i++) {
            if ($this->formfilter[$i] == 1) {
                $countfilter++;
                if ($this->formtype[$i] == 0) {
                    $fm->label($this->formtitle[$i], "form-label")
                        ->input()
        ...
        if ($countfilter > 0) {
            $fm->input()
                ->inptype("submit")
                ->inpclasses("btn btn-success rounded m-2")
                ->inpval("جستجو")
                ->end();
            $fm->input()
                ->inptype("hidden")
    ...
    function submit()
    {
        $this->input()
            ->inptype("submit")
            ->inpval("ثبت")
            ->inpclasses("btn btn-success m-2 rounded")
            ->end();
    }
    ...
    public function pageslist($countitems, $thisurl, $sql)
    {
        ...
        for ($i = 1; $i <= $pagecount; $i++) {
            ...
            $resbtn .= "<input class='btn btn-secondary' style='margin:2px;' type='button' value='" . $i . "' onclick='location.replace(" . '"' . $urlbtn . '"' . ");'>";
        }
    ...
    {
        $this->label($label, "form-label")
            ->end();
        $this->all .= "<img id='imgplc" . $capname . "' src=''>";
        $this->all .= "<input type='button' class='btn btn-secondary rounded m-2' value='بروزرسانی' onclick='changecap(" . '"' . $capname . '"' . ")'>";
        $inpname = "inp" . $capname;
        $inpcls = "cls" . $inpname;
        $this->input()
            ->inpname($inpname)
