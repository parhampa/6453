...
<!DOCTYPE html>
<html>
<title>ویرایش پروفایل کافه</title>
<meta charset="UTF-8">
...
<style>
    ...
    /* ============================================================
       مودال سفارشی (بدون وابستگی به Bootstrap)
       ============================================================ */
...
$dbc = new database();
$dbc->connect()->query("select * from `cafes` where `manager_mobile`='$mob' limit 1");
$fildc = mysqli_fetch_assoc($dbc->res);
?>
            ...
            <div class="panel-card">
                ...
                <div class="card-body p-4">
                    ...
                    <form id="profile-form" onsubmit="return false;">
                        ...
                        <label class="form-label">عنوان کافه</label>
                        <input type="text" name="title" id="title"
                               class="form-control mb-3 required-field datasender"
                               value="<?= htmlspecialchars($fildc['title'] ?? '', ENT_QUOTES, 'UTF-8') ?>">

                        <label class="form-label">شعار کافه</label>
                        <input type="text" name="slogan" id="slogan"
                               class="form-control mb-3 datasender"
                               value="<?= htmlspecialchars($fildc['slogan'] ?? '', ENT_QUOTES, 'UTF-8') ?>">

                        <label class="form-label">شماره تماس ۱</label>
                        <input type="text" name="tel1" id="tel1"
                               class="form-control mb-3 datasender"
                               value="<?= htmlspecialchars($fildc['tel1'] ?? '', ENT_QUOTES, 'UTF-8') ?>">

                        <label class="form-label">شماره تماس ۲</label>
                        <input type="text" name="tel2" id="tel2"
                               class="form-control mb-3 datasender"
                               value="<?= htmlspecialchars($fildc['tel2'] ?? '', ENT_QUOTES, 'UTF-8') ?>">

                        <label class="form-label">نام مدیر کافه</label>
                        <input type="text" name="manager_name" id="manager_name"
                               class="form-control mb-3 required-field datasender"
                               value="<?= htmlspecialchars($fildc['manager_name'] ?? '', ENT_QUOTES, 'UTF-8') ?>">

                        <label class="form-label">آدرس کافه</label>
                        <textarea name="address" id="address" rows="3"
                                  class="form-control mb-3 datasender"><?= htmlspecialchars($fildc['address'] ?? '', ENT_QUOTES, 'UTF-8') ?></textarea>

                        <label class="form-label">اینستاگرام</label>
                        <input type="text" name="instagram" id="instagram"
                               class="form-control mb-3 datasender"
                               value="<?= htmlspecialchars($fildc['instagram'] ?? '', ENT_QUOTES, 'UTF-8') ?>">

                        <label class="form-label">ساعت کاری</label>
                        <input type="text" name="working_hours" id="working_hours"
                               class="form-control mb-3 datasender"
                               value="<?= htmlspecialchars($fildc['working_hours'] ?? '', ENT_QUOTES, 'UTF-8') ?>">
        ...
        $.ajax({
            ...
            success: function (raw) {
                ...
                try {
                    ...
                    showMsg('error', 'پاسخ سرور (دیباگ)', trimmed.substring(0, 500) || '(خالی)');
                    return;
                }

                if (parseInt(resjs.status, 10) === 1) {
                    showMsg('success', 'انجام شد', resjs.msg);
                } else {
