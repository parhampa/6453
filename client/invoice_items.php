        ...
        <div class="col-md-10 col-lg-8">
            <div class="panel-card">
                ...
                <div class="card-body p-4">
                    ...
                    $cfid = get_cafe_id();
                    $fm->setwhere(" `invoice_id` in(select `id` from `invoices` where `cafe_id`=$cfid) ");
                    $fm->deletewhere(" `invoice_id` in(select `id` from `invoices` where `cafe_id`=$cfid) ");
                    $fm->set_where_edit(" `invoice_id` in(select `id` from `invoices` where `cafe_id`=$cfid) ");
                    ...
                    // کوئری با JOIN برای دریافت تاریخ فاکتور، نام مشتری و نام کافه
                    $sql = "SELECT 
                                i.id AS invoice_id, 
                                i.invoice_date, 
                                c.name AS customer_name, 
                                c.family AS customer_family, 
                                cf.title AS cafe_title
                            FROM `invoices` i
                            LEFT JOIN `customers` c ON c.id = i.customer_id
                            LEFT JOIN `cafes` cf ON cf.id = i.cafe_id
                            where `cafe_id`=$cfid
                            ORDER BY i.id DESC";
                    ...
                    // خواندن آیتم‌ها همراه با قیمت
                    $item_prices = [];
                    $sql2 = "SELECT id, title, price FROM `menu_items` where `category_id` in (select `id` from `cafe_categories` where `cafe_id`=$cfid) ORDER BY title";
                    ...
                    // ---- قیمت واحد (اجباری - نمایش در جدول) - نوع: عددی (1) ----
                    ...
                    // ---- تعداد (اجباری - نمایش در جدول) - نوع: عددی (1) ----
...
<!-- ---- اسکریپت پر کردن خودکار قیمت ---- -->
<script>
    // نقشه قیمت آیتم‌ها
    var itemPrices = <?php echo json_encode($item_prices, JSON_UNESCAPED_UNICODE); ?>;

    $(document).ready(function () {
        // ذخیره قیمت اصلی در data-price برای جلوگیری از دستکاری کاربر
        function fillPrice() {
        ...
        // در هنگام تغییر آیتم منو
        $('#menu_item_id').on('change', fillPrice);

        // در هنگام لود اولیه (اگر مقدار از قبل ست شده باشد)
        fillPrice();

        // هشدار اگر قیمت دستی تغییر کند
