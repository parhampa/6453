...
<!DOCTYPE html>
<html>
<title>تعریف دسته بندی کافه</title>
<meta charset="UTF-8">
        ...
        <div class="col-md-10 col-lg-8">
            <div class="panel-card">
                <div class="card-header">
                    تعریف دسته بندی کافه
                </div>
                <div class="card-body p-4">
                    <?php
                    $fm = new makeform();
                    $fm->set_tbl_key("cafe_categories", "id", 1, "تعریف دسته بندی کافه‌ها");
                    $fm->CSRF_token();

                    // عنوان دسته بندی (اجباری - نمایش در جدول)
                    $fm->label("عنوان دسته بندی", "form-label")
                        ->input()
                        ->inpname("title")
                        ->inpid("title")
                        ->inptype("text")
                        ...
                        ->end()
                        ->sndform("title", 0, 1, "عنوان دسته بندی", 1, 1);

                    // تصویر دسته بندی (اختیاری - عدم نمایش در جدول)
                    $fm->fileinput("تصویر دسته بندی (اختیاری)", "image", "form-control mb-3", "form-label", 0);

                    // کافه مربوطه (اجباری - انتخاب از جدول cafes)
                    $fm->label("کافه مربوطه", "form-label")
                        ->select()
                        ->selectname("cafe_id")
                        ->selectid("cafe_id")
                    ...
                    $fm->end()
                        ->sndform("cafe_id", 2, 1, "کافه مربوطه", 1, 1);

                    // دکمه ثبت
                    $fm->input()
                        ->inptype("submit")
                        ->inpval("ثبت دسته بندی")
